...
import uk.co.jcox.molglide.AppConfig
...
class EditorPanel(private val uiData: IDataModelUI) : JPanel() {
    ...
    private fun preparePainter(g2d: Graphics2D) {
        if (AppConfig.settings.componentAntialiasing) g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON)
        if (AppConfig.settings.textAntialiasing) g2d.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON)
        g2d.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS, RenderingHints.VALUE_FRACTIONALMETRICS_ON)
        g2d.font = Font(AppConfig.settings.editorFont, Font.PLAIN, -5)
